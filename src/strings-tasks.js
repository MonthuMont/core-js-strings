...
function removeFirstOccurrences(str, value) {
  ...
  // If the substring is found, remove it using string concatenation
  if (index !== -1) {
...
function sumOfCodes(str) {
  if (typeof str !== 'string') {
    // Return 0 for undefined or non-string input
    return 0;
  }

  // Iterate through each character in the string and sum the character codes
  let sum = 0;
...
function formatTime(minutes, seconds) {
  ...
  // Use padStart to ensure that the minutes and seconds have two digits
...
function findLongestWord(sentence) {
  ...
  // Remove non-alphanumeric characters and get word lengths
  const wordLengths = words.map(
    (word) => word.replace(/[^a-zA-Z]/g, '').length
  );

  // Find the index of the longest word's length
  const maxLengthIndex = wordLengths.indexOf(Math.max(...wordLengths));

  // Use the index to get the longest word from the original array
  const longestWord = words[maxLengthIndex];
...
function invertCase(str) {
  const invertedString = str
    .split('')
    .map((char) => {
      // Check if the character is uppercase and change to lowercase, or vice versa
...
function extractNameFromTemplate(value) {
  const trimmedValue = value.slice('Hello, '.length, -1);
...
function extractEmails(str) {
  ...
  // Trim each email in the array to remove leading and trailing whitespaces
...
function getCardId(value) {
  ...
  if (index === -1) {
    throw new Error(`Invalid id for card '${value}'`);
  }
